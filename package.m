...
(* Viene usata la Dynamic per mostrare l'esito (CORRETTO o SBAGLIATO) del valore inserito dall'utente con un colore diverso (VERDE o ROSSO)*)
  Row[{
    ...
    x = random[[1]] Degree;
    sinCosList2 = DeleteCases[sinCosList, Sin[x]]; (*Cancella dalla lista dei risultati il valore del seno*)
...
TipoAngolo[] := Manipulate[
  Row[{
    ...
    Column[{"\t"}],
    Column[{
    (*Definizione di gradi il cui corpo \[EGrave] all'inzio del package.m*)
      Row[{Text[Style["GRADI (\[Degree])", Blue, Bold, 20]], "\n", Text[Style[defGradi, 17]]}],
      Row[{"\n"}],
    (*Definizione di radianti il cui corpo \[EGrave] all'inzio del package.m*)
...
DisegnaCirconferenza[] := Manipulate[
  ...
  (*inzio graphics*)
    ...
    (*arco identificato dal valore della x della manipulate*)
      {Blue, Circle[{0, 0}, 0.2, {0, x}]},
    (*raggio tratteggiato il cui secondo punto \[EGrave] calcolato in
...
StyleBox[\"\[Alpha]\", \"TradFormChar\"]\)", Red, 20], {x + 20, -30}]
      ...
      (*testo per spiegazione dell'applicazione*)
        s1 = Style["Lorenzo, dopo questa lettura, ha avuto un'idea\nsu come calcolare facilmente l'altezza della tour Eiffel.\nProcede in questo modo: \nmisura la sua distanza dalla base della torre\ne approssima l'angolo \[Alpha] ", 20, font];
        s2 = Style[Hyperlink["(vedi ottante).", "https://it.wikipedia.org/wiki/Ottante_(strumento)"], font, 20];
        Text[StringForm["````", s1, s2]],
        Text[Style["Osserva che si forma un triangolo rettangolo\ne pu\[OGrave] quindi usare la propriet\[AGrave] della tangente.", font, 20]]
      }]
...
(* Gestione della disposizione degli elementi attraverso l'uso di Colonne e Righe annidati *)
    Column[{
            ...
            (* DynamicMoudule rappresenta un oggetto che mantiene le istanze locali passate come argomenti durante
              ...
              (* Rappresentazione di un ogetto grafico di tipo dinamico *)
                Dynamic@Graphics[
                  ...
                  (* poichè risulta graficamente pesante rappresentare il fiume valutando la funzione interpolata in ogni punto
                    nell'intervallo [-21,21], si è scelto di rappresentare il fiume avvalendosi di alcuni punti casuali in cui
                    viene valutata la funzione riverFunction *)
                  ...
                  (* Inset ci permette di inserire un oggetto in un Graphocs
                    ...
                    Inset[
                      ...
                      (* 2.4 rappresenta la distanza da tenere dal centro della linea che rappresenta il fiume *)
      ...
      (* testo di spiegazione dell'applicazione *)
        ...
        Column[{
          Row[{
            Text[Style["\n\nSupponiamo di voler calcolare la distanza fra due punti B e C:\nio mi trovo in C ma non posso raggiungere B perch\[EAcute] \[EGrave] al di l\[AAcute] del fiume.", font, 20]]
          }],
          Row[{
            Text[Style["Possiamo spostarci in un punto A e calcolare la distanza AC\ned inoltre gli angoli CAB=\[Alpha] e ACB=\[Beta].", font, 20]]
          }],
          Text["\n"],
          Row[{
          ...
          Row[{
            Style[
              StringForm[" BC = ``",
                HoldForm["AC Sen(\[Alpha])" / "Sen(\[Gamma])" ]
              ], 20, Bold, font, Blue]
          }]
        }]
      }],
...
(*Esercizio per la conversione da radianti a gradi*)
angleList = List[0, 30, 45, 60, 90, 120, 135, 150, 180, 210, 225, 240, 270, 300, 315, 330, 360];
EsRadiantDegree[] := DynamicModule[{esito = "", colorEsito = Black, randomAngleType},
  randomAngleType = RandomInteger[{0, 1}]; (*A seconda che il valore sia 0 o 1 si avr\[AGrave] una conversione gradi-radianti o viceversa*)
  random = RandomSample[angleList, 4]; (*Sceglie 4 elementi a caso da angleList*)
  incorrects = random[[2 ;; 4]]; (*gli elementi da 2 a 4 nella lista sono quelli non corretti*)
  correct = First[random]; (*il primo \[EGrave] quello corretto*)

  (*Coseno e seno dell'angolo*)
  c := Cos[fromDegreeToPi[correct]];
  s := Sin[fromDegreeToPi[correct]];

  Row[{

    (*Funzione per la stampa a video dell'angolo *)
    Graphics[{
      Circle[],
      Point[{0, 0}],
      PointSize[Large],
      Circle[{0, 0}, 0.2, {0, fromDegreeToPi[correct]}],
      {Dashed, Line[{{0, 0}, {c, s}}]}, (*raggio*)
      Thickness[0.01],
      RGBColor[0, 255, 0],
      Line[{{0, 0}, {c, 0}}],
      RGBColor[255, 0, 0],
    ...
    Column[{
      (*crea i pulsanti per le risposte, diversi a seconda se sia la conversione da gradi a radianti o viceversa*)
      ...
      (*I pulsanti vengono mescolati per fare in modo che la risposta corretta non si trovi sempre nella stessa posizione*)
      buttons = List[btn5, btn6, btn7, btn8];
      randomButtons = RandomSample[buttons];
    ...
    (*Domanda dell'esercizio diversa a seconda del tipo di conversione da effettuare*)
      If[randomAngleType == 0,
...
(* Funzione per controllare se la risposta all'esercizietto \[EGrave] corretta *)
  checkRisp[risp_, correct_] := (
          ...
          (* DynamicMoudule rappresenta un oggetto che mantiene le istanze locali passate come argomenti durante
            ...
            DynamicModule[{},
              ...
              (* Parte di prato al di sopra del fiume delimitata da due rette *)
                ...
                (* Setto colore e opacit\[AGrave] del prato *)
              ...
              Questi sono contenuti in una inset perch\[EAcute] si evita un fastidioso errore *)
                Inset[
              ...
              (* Style dell'angolo \[Alpha]\:2081 che si muove in base allo spostamento del locator relativo a A *)
                Style[Text[Subscript["\[Alpha]", 1], {First[p4], Last[p4] + 0.4}], 15],
              (* Style dell'angolo \[Alpha]\:2082 che si muove in base allo spostamento del locator relativo a A *)
              ...
              (* Style dell'angolo \[Gamma]\:2081 che si muove in base allo spostamento del locator relativo a C *)
                Style[Text[Subscript["\[Gamma]", 1], {First[p1] + 0.4, Last[p1] + 0.1}], 12],
              (* Style dell'angolo \[Gamma]\:2082 che si muove in base allo spostamento del locator relativo a C *)
              ...
              (* Style dell'angolo \:d771 che si muove in base allo spostamento del locator relativo a B *)
              ...
              (* Style dell'angolo \[Delta] che si muove in base allo spostamento del locator relativo a D *)
                Style[Text["\[Delta]", {First[p3] - 0.35, Last[p3] - 0.3}], 15],

              (* Visualizzazione ampiezza degli angoli: \[Alpha]\:2081, \[Alpha]\:2082, \[Gamma]\:2081, \[Gamma]\:2082
              ...
              (* Archetti che delimitano gli angoli \[Alpha]\:2081, \[Alpha]\:2082, \[Gamma]\:2081, \[Gamma]\:2082, ottenuti usando la primitiva Circle,
              che prende come argomenti:
                                          ...
                                          - gli angoli \[Theta]\:2081 e \[Theta]\:2082, che definiscono i due punti su cui \[EGrave] definito ogni arco;

              Si \[EGrave] fatto uso della funzione getAngle che prende come argomenti tre punti e ritorna l'ampiezza dell'angolo in radianti.
              Per evitare problemi di visualizzazione in alcune circostanze sono stati valutati casi limite,
              calcolando \[Theta]\:2081 e \[Theta]\:2082 come somma di angoli a partire da 0, \[Pi]/2, \[Pi], 3/2*\[Pi]
              *)
  ...
  e aiuti racchiusi nelle finestre popup innescate dai pulsanti *)
    Column[{
      ...
      Row[{
        ...
        Button["Calcolare BD",
          CreateDialog[{
            TextCell["\[CapitalEGrave] possibile calcolare la lunghezza di BD applicando Carnot:\n\n\tBD = \[Sqrt]AB+AD-2*AB*AD*cos(\[Alpha]1-\[Alpha]2)"],
            DefaultButton[]
          }],
          ImageSize -> Medium, BaseStyle -> {"GenericButton"}
        ],
